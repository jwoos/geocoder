		...
		geocoder.geocode({'address': address}, function(results, status) {
			if (status === google.maps.GeocoderStatus.OK) {
				...
				if (returned.length > 1) {
					...
					console.log(returned[0].geometry.location.lat() + ',' + returned[0].geometry.location.lng());
					document.getElementById('latitude').value = returned[0].geometry.location.lat();
					document.getElementById('longitude').value = returned[0].geometry.location.lng();
					document.getElementById('coordinate').value = returned[0].geometry.location.lat() + ',' + returned[0].geometry.location.lng();
				}
			} else {
